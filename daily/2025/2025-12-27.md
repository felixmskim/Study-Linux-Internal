...
- **원인:**
  1. **VS Code Server의 인덱싱 작업** : Remote-WSL을 사용하면 WSL 내부에 VS Code Server를 설치한다. 특히 리눅스 커널처럼 파일이 수만 개인 프로젝트를 열면, IntelliSense(코드 자동 완성, 정의 이동 등) 기능을 제공하기 위해 모든 파일을 스캔하고 인덱싱한다. 이때 CPU 사용량이 급증한다.
...
- **해결:** WSL 자원을 제한한다. 윈도우 사용자 폴더 ```(C:\Users\사용자이름)```에 ```.wslconfig``` 파일을 만들어 WSL이 사용할 수 있는 최대 메모리와 CPU 코어 수를 제한할 수 있다. 참고로 밑의 명령어는 PowerShell 기준이다.
...
- 이는 실시간(RT) 프로세스와 일반 프로세스를 동시에 처리하기 위한 설계임을 깨달았다.
